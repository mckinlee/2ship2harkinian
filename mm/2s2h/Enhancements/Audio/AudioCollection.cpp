...
AudioCollection::AudioCollection() {
    ...
    for (const auto& [seqId, seqInfo] : mSequenceMap) {
        ...
        if (!(seqInfo.category == SEQ_BGM_CUSTOM)) {
            ...
            if (seqInfo.category & SEQ_BGM_WORLD) mSequenceTypeCounts[SEQ_BGM_WORLD]++;
            if (seqInfo.category & SEQ_BGM_EVENT) mSequenceTypeCounts[SEQ_BGM_EVENT]++;
            if (seqInfo.category & SEQ_BGM_BATTLE) mSequenceTypeCounts[SEQ_BGM_BATTLE]++;
            if (seqInfo.category & SEQ_OCARINA) mSequenceTypeCounts[SEQ_OCARINA]++;
            if (seqInfo.category & SEQ_FANFARE) mSequenceTypeCounts[SEQ_FANFARE]++;
            if (seqInfo.category & SEQ_SFX) mSequenceTypeCounts[SEQ_SFX]++;
            if (seqInfo.category & SEQ_INSTRUMENT) mSequenceTypeCounts[SEQ_INSTRUMENT]++;
            if (seqInfo.category & SEQ_VOICE) mSequenceTypeCounts[SEQ_VOICE]++;
        }
    }
}
